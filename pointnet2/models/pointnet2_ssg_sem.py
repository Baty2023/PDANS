...
class PointNet2SemSegSSG(nn.Module):
    ...
    def build_SA_model(
        ...
        for i in range(len(npoint)):
            ...
            if i==0:
                mlp_pre = [in_fea_dim, in_fea_dim, in_fea_dim, in_fea_dim]
            else:
                if additional_fea_dim is None:
                    mlp_pre = [feature_dim[i-1]]*mlp_depth + [feature_dim[i]]
                else:
                    # mlp_spec = [ feature_dim[i]+additional_fea_dim[i] ]*mlp_depth + [feature_dim[i+1]]
                    mlp_pre = [feature_dim[i-1]]*mlp_depth + [feature_dim[i]]
                    mlp_pre[0] = mlp_pre[0] + additional_fea_dim[i-1]
    ...
    def build_FP_model(self, feats_dim, decoder_feature_dim, decoder_mlp_depth, feature_dim, in_fea_dim, include_t, 
        ...
        for i in range(len(decoder_feature_dim)-1):
            ...
            if use_knn_FP:
                ...
                use_global_attention = ((not global_attention_setting is None) and 
                                    ...
                                    i in global_attention_setting['global_attention_layer_index'])
                this_global_attention_setting = global_attention_setting if use_global_attention else None
                next_point = [1024, 256, 64, 64]
                degrees = [1, 4, 4, 1]
                FP_modules.append(PointnetKnnFPModule(inx=i,feats_dim=feats_dim, next_point=next_point, mlp1=mlp1_spec, mlp2=mlp2_spec, K=K, first_conv=False, bn=bn,
                            ...
                            include_second_condition=include_second_condition, second_condition_dim=second_condition_dim,
                                                    ...
                                                    attention_setting=attention_setting,
                                                    global_attention_setting=this_global_attention_setting, degrees=degrees))
            else:
                # if additional_fea_dim is None:
