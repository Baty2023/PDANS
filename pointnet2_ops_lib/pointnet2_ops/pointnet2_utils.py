...
class GruopFeature(nn.Module):
    ...
    def forward(self, xyz, vertices=None, feature_map=None,
        ...
        if self.neighbor_def == 'radius':
            ...
            normalized_xyz = neighbor_displacement.permute(0, 3, 1, 2)  
            center_coords = vertices.permute(0, 2, 1).unsqueeze(-1)  
            transformed_neighbor = self.conv_kv(normalized_xyz)  
            transformed_new_point = self.conv_q(center_coords)   

            transformed_feature1 = transformed_neighbor[:, :self.bottleneck_channel, :, :]  
            feature = transformed_neighbor[:, self.bottleneck_channel:, :, :]  
