...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: Optional[torch.Tensor], 
        ...
        if N < self.npoint:
            scale = self.npoint / N
            new_xyz =  midpoint_interpolate(xyz_flipped, scale).transpose(1, 2).contiguous()
            if self.use_attention_module:
                new_xyz_feat = midpoint_interpolate(features, scale)
                new_xyz_feat = new_xyz_feat.transpose(1, 2).contiguous()
        else:
            scale = N / self.npoint
            sample_idx = torch.randperm(N)[:self.npoint]
            new_xyz = xyz[:, sample_idx, :]
            if self.use_attention_module:
                features = features.transpose(1, 2).contiguous()
                neighbor_index = get_neighbor_index(features, 8)
                neighbor_feature = indexing_neighbor(
                                    features,
                                    neighbor_index)
                pooled_feature = torch.max(neighbor_feature, dim=2)[0] 
                new_xyz_feat = pooled_feature[:, sample_idx, :] 
        ...
        for i in range(len(self.groupers)):
            grouped_features, count = self.groupers[i](xyz, vertices=new_xyz, feature_map=new_xyz_feat, subset=subset, 
            ...
            if self.use_attention_module:
                new_features = self.attention_modules[i](new_xyz_feat.transpose(1, 2).contiguous(), grouped_features, out_features, count)
                # (B, mlp[-1], npoint)
            else:
    ...
    def __init__(self, npoint, radii, nsamples, mlps, bn=True, use_xyz=True, 
        ...
        for i in range(len(radii)):
            ...
            self.groupers.append(
                pointnet2_utils.GruopFeature(radius, nsample, use_xyz=use_xyz, support_num = 3, 
                                            include_abs_coordinate=include_abs_coordinate,
                                            include_center_coordinate=include_center_coordinate,
                                            neighbor_def=neighbor_def, mlp=mlp_spec, mlp_pre=mlp_pre)
...
class PointnetSAModule(PointnetSAModuleMSG):
    ...
    def __init__(self, mlp, mlp_pre=None, npoint=None, radius=None, nsample=None, bn=True, use_xyz=True,
...
class TreeGCN(nn.Module):
    def __init__(self, depth, features, out_channel, degrees, support, node, upsample=False, activation=True):
        self.depth = depth
        self.in_feature = features[5-depth]
        self.out_feature = out_channel
        self.node = node
        self.degree = degrees[depth-1]
        self.upsample = upsample
        self.activation = activation
        super(TreeGCN, self).__init__()

        self.W_root = nn.ModuleList([nn.Linear(features[inx], self.out_feature, bias=False) for inx in range(-self.depth, 0)])
    ...
    def forward(self, tree):
        root = 0
        for inx in range(-self.depth, 0):
            root_num = tree[inx].size(2)
            repeat_num = int(self.node / root_num)
            root_node = self.W_root[inx](tree[inx].permute(0, 2, 1))
            self.batch = root_node.shape[0]
            root = root + root_node.repeat(1,1,repeat_num).view(self.batch,-1,self.out_feature)

        branch = 0
        if self.upsample:
            branch = tree[-self.depth].permute(0, 2, 1).unsqueeze(2) @ self.W_branch
...
class PointnetKnnFPModule(nn.Module):
    ...
    def __init__(self, inx, feats_dim, next_point, mlp1, mlp2, K, bn=True, t_dim=128, include_t=False,
                    bn_first=False, bias=False, first_conv=False,
                    ...
                    neighbor_def='radius', activation='relu',
                    attention_setting=None,
                    global_attention_setting=None, degrees=None):
        # type: (PointnetFPModule, List[int], bool) -> None
        ...
        out_channel = mlp1[0]
        support = 10
        self.tree = TreeGCN(depth, feats_dim, out_channel, degrees, support, node, upsample=True, activation=True)
    ...
    def forward(self, features, unknown, known, unknow_feats, known_feats, t_emb=None, condition_emb=None, 
        ...
        if known is not None:
            grouped_feats = self.tree(features)
            ...
            grouped_feats = grouped_feats.permute(0, 2, 1).unsqueeze(3)
            second_condition_emb = second_condition_emb if self.include_second_condition else None
            ...
            if self.use_attention_module:
                ...
                if unknown.shape[1] != grouped_feats.shape[2]:
                    grouped_feats = F.interpolate(grouped_feats, size=(unknown.shape[1], 1), mode='bilinear', align_corners=False)
                    grouped_feats_out = F.interpolate(grouped_feats_out, size=(unknown.shape[1], 1), mode='bilinear', align_corners=False)
